...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun instantiateViewModels() {
        ...
        ViewModelProvider(this, ViewModelFactory(ProfileRepository(profileApi))).get(
        ...
        ViewModelProvider(this, ViewModelFactory(DashBoardRepository(dashboardApi))).get(
            DashboardViewModel::class.java
        )
        ViewModelProvider(
            this,
            ViewModelFactory(SettingsRepository())
        ).get(SettingsViewModel::class.java)
        val vendorApi = retrofitClient.buildApi(VendorsApi::class.java, token)
        ViewModelProvider(
            this,
            ViewModelFactory(VendorRepository(vendorApi))
        ).get(VendorViewModel::class.java)
        val accessControlApi = retrofitClient.buildApi(AccessControlApi::class.java, token)
        ViewModelProvider(this, ViewModelFactory(AccessControlRepository(accessControlApi))).get(
            AccessControlViewModel::class.java
        )
        val categoryApi = retrofitClient.buildApi(CategoryApi::class.java, token)
        ViewModelProvider(this, ViewModelFactory(CategoryRepository(categoryApi))).get(
            CategoryViewModel::class.java
        )
        val locationApi = retrofitClient.buildApi(LocationApi::class.java, token)
        ViewModelProvider(this, ViewModelFactory(LocationRepository(locationApi))).get(
            LocationViewModel::class.java
        )
        val lotApi = retrofitClient.buildApi(LotApi::class.java, token)
        ViewModelProvider(
            this,
            ViewModelFactory(LotRepository(lotApi))
        ).get(LotViewModel::class.java)
        val classificationApi = retrofitClient.buildApi(ClassificationApi::class.java, token)
        ViewModelProvider(this, ViewModelFactory(ClassificationRepository(classificationApi))).get(
